...
```{r F2, fig.width=7, fig.height= 3.5, warning = FALSE, message = FALSE}
library(ggplot2)
...
5. Based on your results from 3. and 4. and assuming the null hypothesis is true, about how frequently do you think you would obtain a mean difference as large as or larger than 7.6 by random allocation alone?

6. Does your answer from 5. lead you to believe the "chance alone" position (i.e., the null hypothesis that the mean worm count is the same for both the treatment and the control), or does is lead you to believe that K11777 has a positive inhibitory effect on the schistosome worm in female mice?  Explain.
...
```{r EXPLAIN, message = FALSE, warning = FALSE}
